...
axesDD = uidropdown(xg, ...
    'Items', {'as is', ...
              'rotate +90 about X  (Y-up -> Z-up)', 'rotate -90 about X  (Z-up -> Y-up)', ...
              'rotate +90 about Y', 'rotate -90 about Y', 'rotate +90 about Z', 'rotate -90 about Z', ...
              'mirror: swap X / Y', 'mirror: swap X / Z', 'mirror: swap Y / Z', ...
              'mirror: flip X', 'mirror: flip Y', 'mirror: flip Z', 'custom ...'}, ...
    'ItemsData', {'X Y Z', 'X -Z Y', 'X Z -Y', 'Z Y -X', '-Z Y X', '-Y X Z', 'Y -X Z', ...
                  'Y X Z', 'Z Y X', 'X Z Y', '-X Y Z', 'X -Y Z', 'X Y -Z', 'custom'}, ...
    'Value', 'X Y Z', ...
    'Tooltip', sprintf(['Axis map applied to the cloud: which signed CLOUD axis becomes each MODEL axis.\n' ...
                        'Rotations keep handedness (a rotated thermal model); mirrors flip it (a mirrored\n' ...
                        'half-model). Same silhouette either way -- pick what was actually done.\n' ...
...
parCB = put(uicheckbox(g3, 'Text', 'Parallel steps', 'Value', false, ...
            'Tooltip', sprintf(['Map the steps after the first in a parfor (Parallel Computing Toolbox). The first step\n' ...
                                'of each part builds the mapping; the rest reuse it, so the CSV read is what gets parallelised.']), ...
            'ValueChangedFcn', @(~, ~) save_prefs()), 2, [1 2]);
shellCB = put(uicheckbox(g3, 'Text', 'Shells: average through thickness', 'Value', true, ...
              'Tooltip', sprintf(['Grids on CQUAD4 / CTRIA3 (...) elements: sample the cloud at 5 levels across the plate\n' ...
                                  'thickness (PSHELL T or PCOMP total) along the grid normal and write the average,\n' ...
                                  'instead of the mid-plane value. Solid grids are unaffected.']), ...
              'ValueChangedFcn', @(~, ~) save_prefs()), 2, [3 4]);
...
g4 = uigridlayout(p4, [7 4]);
g4.ColumnWidth = {60, '1x', 90, 64}; g4.RowHeight = {24, 24, 24, 24, 24, 22, 28};
g4.Padding = [8 4 8 4]; g4.RowSpacing = 5;

put(uilabel(g4, 'Text', 'Output'), 1, 1);
outE = put(uieditfield(g4, 'text', 'Value', 'temp_cards', ...
           'Tooltip', 'One TEMP file per time step lands here (created if missing).', ...
           'ValueChangedFcn', @(~, ~) save_prefs()), 1, [2 3]);
put(uibutton(g4, 'Text', 'Browse', 'ButtonPushedFcn', @on_browse_out), 1, 4);
put(uilabel(g4, 'Text', 'File name'), 2, 1);
nameE = put(uieditfield(g4, 'text', 'Value', 'temp_{index:03}', ...
            'Tooltip', sprintf(['File name per step, no extension. Tokens: {file} {time} {sid} {index};\n' ...
                                '{index:03} / {sid:04} zero-pad.  e.g. temp_{index:03} -> temp_001.bdf']), ...
            'ValueChangedFcn', @(~, ~) save_prefs()), 2, [2 4]);

put(uilabel(g4, 'Text', 'First SID'), 3, 1);
sidE = put(uieditfield(g4, 'numeric', 'Value', 1, 'Limits', [1 Inf], 'RoundFractionalValues', 'on', ...
           'Tooltip', 'Load set ID of the first CSV; the rest count up in list order. Applied at Load & Map.'), 3, 2);
put(uilabel(g4, 'Text', 'Card format', 'HorizontalAlignment', 'right'), 3, 3);
fieldDD = put(uidropdown(g4, 'Items', {'small (8)', 'large (16)'}, 'ItemsData', [8 16], 'Value', 8, ...
              ...
              'ValueChangedFcn', @(~, ~) save_prefs()), 3, 4);
...
extraE = put(uieditfield(g4, 'text', 'Value', '', 'Placeholder', 'SPC = 1 ; DISP(PLOT) = ALL', ...
             ...
             'ValueChangedFcn', @(~, ~) save_prefs()), 5, 2);
put(uilabel(g4, 'Text', 'T ref', 'HorizontalAlignment', 'right', ...
            'Tooltip', 'Reference (stress-free) temperature in model units -> TEMPD + global TEMPERATURE(INITIAL). Blank = none.'), 5, 3);
trefE = put(uieditfield(g4, 'text', 'Value', '', 'Placeholder', 'none', ...
            ...
            'ValueChangedFcn', @(~, ~) save_prefs()), 5, 4);
...
reportCB = put(uicheckbox(g4, 'Text', 'HTML report', 'Value', true, ...
               ...
               'ValueChangedFcn', @(~, ~) save_prefs()), 6, [1 2]);
pngCB = put(uicheckbox(g4, 'Text', 'PNG per step (slow at 1M)', 'Value', false, ...
            ...
            'ValueChangedFcn', @(~, ~) save_prefs()), 6, [3 4]);

wr = uigridlayout(g4, [1 3]); put(wr, 7, [1 4]);
...
% --- setup files ---------------------------------------------------------------
    ...
    function apply_setup(st)
        ...
        nameE.Value      = f('outname', 'temp_{index:03}');
...
% --- preferences -------------------------------------------------------------
    function load_prefs()
        ...
        nameE.Value       = getpref(PREF, 'outname',  'temp_{index:03}');
