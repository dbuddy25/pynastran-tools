...
C.CASE_EXTRA     = {};              % lines repeated in every subcase, e.g. {'SPC = 1', 'DISP(PLOT) = ALL'}
C.TREF           = [];              % reference temperature in OUT_UNITS: writes a TEMPD
                                    % card (SID = TREF_SID) and TEMPERATURE(INITIAL) in
                                    % every subcase.  [] = none
C.TREF_SID       = 99999;
...
%   plus the optional reference-temperature TEMPD), both in OUT_DIR.
    ...
    if ~isempty(C.TREF)
        fprintf(fid, '$ Reference temperature %.3f %s on TEMPD SID %d (see temp_includes.bdf)\n', ...
    ...
    for k = 1:n
        ...
        if ~isempty(C.TREF),     fprintf(fid, '  TEMPERATURE(INITIAL) = %d\n', C.TREF_SID); end
